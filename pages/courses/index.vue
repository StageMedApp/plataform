    ...
    <div class="container">
      <div class="grid sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-x-[3%] gap-y-6 w-full">
        <NuxtLink :to="'/courses/' + course._id" class="w-full relative cursor-pointer" v-for="course in courses">
          <img class="w-full rounded-2xl aspect-video" :src="course.cover" />
            ...
            <div>
              ...
              <div class="mt-1 flex flex-col">
                <div class="flex gap-y-1 gap-x-4 flex-wrap">
                  ...
                  <p class="text-body-sm text-neutral tracking-wide flex items-center gap-1.5">
                    <Icon name="solar:chat-square-check-line-duotone" /> Comentarios - {{ course.count.commets }}
                  </p>
                </div>
                <p class="text-body-sm text-neutral-800 font-medium tracking-wider">
                  {{ getDayRelative(course.created_at) }} atrás
                </p>
              </div>
            </div>
          </div>
