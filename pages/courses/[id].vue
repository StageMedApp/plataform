    ...
    <div class="container">
      <div class="grid lg:grid-cols-3 gap-y-6 gap-x-7">
        <div class="w-full lg:col-span-2">
          <div class="w-full aspect-video rounded-lg flex justify-center items-center">
            <VideoPlayer class="w-full aspect-video h-auto rounded-lg overflow-hidden" :options="videoOptions" />
          </div>
          <div class="mb-10">
            ...
            <div class="sm:flex items-start justify-between mt-3 space-y-3">
              <div class="flex flex-col gap-y-1 gap-x-4 flex-wrap">
                ...
                <p class="text-body-sm text-neutral tracking-wide flex items-center gap-1.5"><Icon name="solar:chat-square-check-line-duotone" /> Comentarios - {{ comments.length }}</p>
              </div>
              ...
              <ul class="divide-y divide-gray-200">
                <li class="pb-3 sm:pb-4" v-for="file in course.files" :key="file.name">
                  <div class="flex items-center space-x-4">
                    ...
                    <Button icon color="link" size="small" @click="downloadFile(file.file, file.name)"> <Icon name="solar:download-square-line-duotone" size="20" /> </Button>
                  </div>
                </li>
              </ul>
            </DisclosurePanel>
        ...
        <div class="flex flex-col">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg lg:text-2xl font-bold text-gray-900">Comentários ({{ comments.length }})</h2>
          </div>
          ...
          <div class="p-3 mb-6 text-base bg-white rounded-lg" v-for="comm in comments" :key="comm._id">
            <div class="flex justify-start items-center mb-2">
              <p class="inline-flex items-center mr-3 text-sm text-gray-900 grow"><img class="mr-2 w-6 h-6 rounded-full" :src="comm.picture" alt="Michael Gough" />{{ comm.name }}</p>
              <p class="text-sm text-gray-600">{{ dayjs(comm.created_at).format("DD/MM/YYYY") }}</p>
            </div>
            <p class="text-gray-500">
              {{ comm.comment }}
            </p>
            <div v-if="comm.reply">
              <p class="pl-6 mt-4 text-gray-900 font-semibold">Resposta</p>
              <p class="pl-6 text-gray-500">
                {{ comm.reply?.text }}
              </p>
            </div>
          </div>
