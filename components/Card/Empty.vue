<template>
  <div id="EmptyCard">
    <img
      loading="lazy"
      v-if="img || typeObj[type].img"
      :src="img || typeObj[type].img"
      :alt="alt || typeObj[type].alt"
    />
    ...
    <Button
      tag="div"
      size="small"
      class="!px-10 w-auto mt-10"
      v-if="action || (typeObj[type].action && !to)"
      @click="callback"
      >{{ action || typeObj[type].action }}</Button
    >
    <Button size="small" class="!px-10 w-auto mt-10" v-else-if="action || (typeObj[type].action && to)" :to="to">{{
